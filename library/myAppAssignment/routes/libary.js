...
router.get('/', function (req, res) {
    ...
    req.db.collection('library').find().toArray()
        .then(data => {
            console.log('get')
...
router.get('/borrowed', function (req, res) {
    req.db.collection('books').find({ Borrowers: { $ne: [] } }).project({ title: 1, Borrowers: 1 }).toArray()
        .then(data => {
            console.log('get')
            res.json({ Status: "Success", data: data })
        })
...
//     req.db.collection('library').findOne({ _id: { $eq: new ObjectID(id) } })
//         .then(data => {
//             console.log('get')
//             res.json({ Status: "Success", data: data })
//         })
